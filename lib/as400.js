/*global define:false*/
define(['underscore', 'fs', 'lib/taskutil', 'lib/couchutil', 'views/lib/codici', 'lib/inventario', 'dbconfig'], function (_, fs, taskutil, couchutil, codici, inventario, dbconfig) {
  'use strict';
  ...
  function buildModelliEScalariniAs400(modelliEScalarini) {
    var col = codici.colNamesToColIndexes(modelliEScalarini[0].columnNames),
      warns = [],
      lista = {};
    modelliEScalarini[0].rows.forEach(function (r) {
      var stagione = r[col.stagione],
  ...
  function buildCostoArticoli(currentYear, costi, analis01, listini) {
    var col = codici.colNamesToColIndexes(analis01[0].columnNames),
      warns = [],
      listino1 = listini['1'],
      log = fs.createWriteStream('sync.log'),
      costiDaAggiungere = fs.createWriteStream('costi-mancanti.tsv');

    function isCurrent(stagione) {
      var year = parseInt(stagione.substring(0, 2), 10);
      return year >= currentYear && year < (currentYear + 10);
    }

    analis01[0].rows.forEach(function (r) {
      var stagione = codici.padZero(r[col.stagione], codici.LEN_STAGIONE),
        modello = codici.padZero(r[col.modello], codici.LEN_MODELLO),
        articolo = codici.padZero(r[col.articolo], codici.LEN_ARTICOLO),
        costo = codici.parseMoney(r[col.costo]);
      if (!(stagione && modello && articolo) || !costo) {
        warns.push('Riga non valida: "' + r.join('" "') + '"');
      } else if (typeof codici.getProperty(costi, stagione, modello, articolo) != 'undefined' || isCurrent(stagione)) {
        codici.setProperty(costi, stagione, modello, articolo, costo);
      }
    });

    // cleanup
    Object.keys(costi).forEach(function (stagione) {
      var costiStagione = costi[stagione];
      Object.keys(costiStagione).forEach(function (modello) {
        var costiModello = costiStagione[modello];
        Object.keys(costiModello).forEach(function (articolo) {
          if (costiModello[articolo] === 0) {
            var costo,
              costi = Object.keys(listino1.prezzi).map(function (s) {
                var prezzi = codici.getProperty(listino1.prezzi, s, modello, articolo);
                return prezzi ? prezzi[listino1.col.costo] : 0;
              }).sort();
            log.write([stagione, modello, articolo, costi.join(' ')].join(' ') + '\n');
            if (costi.length) {
              costo = costi[costi.length - 1];
              costiDaAggiungere.write([stagione, modello, articolo, costo / 100].join('\t') + '\r\n');
            } else {
              warns.push('Manca costo: ' + [stagione, modello, articolo].join(' '));
            }
            delete costiModello[articolo];
            if (Object.keys(costiStagione[modello]).length === 0) {
              delete costiStagione[modello];
            }
          }
        });
      });
    });
    costiDaAggiungere.destroySoon();
    log.destroySoon();

    return [warns, Object.keys(costi).map(function (stagione) {
      return { _id: 'CostoArticoli_' + stagione, costi: costi[stagione] };
    })];
  }
  ...
  return {
    ...
    buildCostoArticoli: buildCostoArticoli,
    ...
    updateCostoArticoliAs400: function (couchdb, callback) {
      getListini(couchdb, function (errListini, listini) {
        if (errListini) {
          return callback(errListini);
        }
        couchdb.view(dbconfig.designDoc, 'giacenze', { group: true }, function (err, giacenze) {
          if (err) {
            return callback(err);
          }
          var costi = {};
          giacenze.rows.forEach(function (row) {
            if (row.value !== 0) {
              var k = row.key, stagione = k[3], modello = k[0], articolo = k[1];
              codici.setProperty(costi, stagione, modello, articolo, 0);
            }
          });

          fetchFromAs400(as400Querier, ['analis01'],
            function (listinoCostiAs400) {
              // FIXME don't hardcode currentYear
              return buildCostoArticoli(12, costi, listinoCostiAs400, listini);
            },
            function (err, warnsAndDocs) {
              if (err) {
                return callback(err);
              }
              var docs = warnsAndDocs[1], i = -1, ii = docs.length, allResps = [], updateNext;

              function save(doc) {
                couchutil.saveIfChanged2(couchdb, doc, function (err, resp) {
                  if (err) {
                    return callback(err);
                  }
                  if (resp) {
              ...
              updateNext = function () {
                i += 1;
                if (i >= ii) {
                  return callback(null, [warnsAndDocs[0]], allResps.length ? allResps : null);
                }
                save(docs[i]);
              };
