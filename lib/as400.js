...
define(['underscore', 'lib/taskutil', 'lib/couchutil', 'app/js/codici', 'lib/listino', 'dbconfig'], function (_, taskutil, couchutil, codici, libListino, dbconfig) {
  'use strict';
  ...
  function buildInventariMagazzinoClientiAs400(codiceAzienda, listini, taglieScalarini, modelliEScalarini, inventarioOrdet00fAs400) {
    ...
    for (i = 0, ii = rows.length; i < ii; i += 1) {
      ...
      if (!desscal) {
        warns.push('ORDET00F MODELLO NON IN ANAGRAFE: ' + [stagione, modello].join(' '));
      } else if (desscal[1] !== scalarino) {
        warns.push('ORDET00F SCALARINO IN ANAGRAFE DIVERSO PER MODELLO ' + [stagione, modello].join(' ') + ': "' + scalarino + '". Scalarino in anagrafe="' + desscal[1] + '"');
      } else {
        ...
        for (j = 0, jj = taglie.length; j < jj; j += 1) {
          ...
          if (qta) {
            ...
            if (!prezzi) {
              warns.push('ORDET00F ARTICOLO NON A LISTINO PER ' + codiceAzienda + ': ' + [stagione, modello, articolo].join(' '));
              costo = COSTO_NULLO;
            } else {
    ...
    return [warns, [
      newInventario(codiceAzienda, 1, codici.TIPO_MAGAZZINO_CLIENTI, 0, inventarioPronto),
      newInventario(codiceAzienda, 2, codici.TIPO_MAGAZZINO_CLIENTI, 1, inventarioInProduzione)
    ]];
  }
  ...
  function buildInventariMagazzinoDisponibileAs400(codiceAzienda, listini, taglieScalarini, modelliEScalarini, inventarioSalmodAs400) {
    ...
    for (i = 0, ii = rows.length; i < ii; i += 1) {
      ...
      desscal = mes[stagione + modello];
      if (!desscal) {
        warns.push('SALMOD MODELLO NON IN ANAGRAFE: ' + [stagione, modello].join(' '));
      } else if (desscal[1] !== scalarino) {
        warns.push('SALMOD SCALARINO IN ANAGRAFE DIVERSO PER MODELLO ' + [stagione, modello].join(' ') + ': "' + scalarino + '". Scalarino in anagrafe="' + desscal[1] + '"');
      } else {
        articolo = codici.padZero(r[col.articolo], codici.LEN_ARTICOLO);
        colore = codici.padZero(r[col.colore], codici.LEN_COLORE);
        //TODO fare anche controllo listino As400.
        ...
        if (qta) {
          ...
          prezzi = codici.readListino(listini, codiceAzienda, stagione, modello, articolo);
          if (!prezzi) {
            warns.push('SALMOD ARTICOLO NON A LISTINO PER ' + codiceAzienda + ': ' + [stagione, modello, articolo].join(' '));
            costo = COSTO_NULLO;
          } else {
            costo = prezzi[1][prezzi[0].costo];
          }
  ...
  return {
    ...
    updateGiacenze: function (couchdb, callback) {
      couchdb.view(dbconfig.designDoc, 'aziende', { include_docs: true }, function (err, aziende) {
        ...
        libListino.getAll(couchdb, function (errListini, listini) {
          if (errListini) {
            ...
            couchdb.get('ModelliEScalarini', function (err, modelliEScalarini) {
              ...
              function saveAll() {
                function saveDocs(docs) {
                  ...
                  var k, kk, doc;
                  for (k = 0, kk = docs.length - 1; k < kk; k += 1) {
                    doc = docs[k];
                    doc.WORK_IN_PROGRESS = 1;
                    couchdb.insert(doc, doc._id, saveDone);
                  }
                  doc = docs[kk];
