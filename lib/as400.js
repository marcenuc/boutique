...
(function () {
  ...
  function buildAziendaAs400(azienda, aziendeAs400) {
    var col = colNamesToColIndexes(aziendeAs400.columnNames),
      az = aziendeAs400.rows[0];
  ...
  exports.buildAziendaAs400 = buildAziendaAs400;


  function fetchFromAs400(queryParameters, docBuilder, callback) {
    var queries = Array.isArray(queryParameters[0]) ? queryParameters : [queryParameters],
      i = 0,
      n = queries.length,
      resps = [];

    function doQuery() {
      execBuffered('java', ['-jar', 'as400-querier.jar'].concat(queries[i]), function (err, docAs400) {
        if (err) {
          return callback(err);
        }
        try {
          resps.push(JSON.parse(docAs400));
        } catch (errJSONParse) {
        ...
        if (i < n) {
          ...
          callback(err, docBuilder.apply(this, resps));
        }
      });
    }
  ...
  exports.updateModelliEScalariniAs400 = function (couchdb, callback) {
    fetchFromAs400(['modelli'], buildModelliEScalariniAs400, function (err, warnsAndDoc) {
      saveIfChanged(err, couchdb, warnsAndDoc, callback);
    });
  };

  exports.updateCausaliAs400 = function (couchdb, callback) {
    fetchFromAs400([['codifiche', 'selector=MC'], ['codifiche', 'selector=MD']],
      buildCausaliAs400,
  ...
  function newBuilderAziendaAs400(azienda) {
    return function (aziendaAs400) {
      return buildAziendaAs400(azienda, aziendaAs400);
    };
  }
  ...
  exports.updateAziendeAs400 = function (couchdb, callback) {
    couchdb.all({
      startkey: '"Azienda_"',
      endkey: '"Azienda_\ufff0"',
      include_docs: true
      ...
      function doSave() {
        ...
        fetchFromAs400(['clienti', 'codice=' + codice],
          newBuilderAziendaAs400(row.doc),
          function (err, warnsAndDoc) {
            saveIfChanged(err, couchdb, warnsAndDoc, function (errSave, warns, resp) {
              if (errSave) {
