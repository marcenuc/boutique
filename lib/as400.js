...
define(['underscore', 'lib/taskutil', 'lib/couchutil', 'app/js/codici'], function (_, taskutil, couchutil, codici) {
  ...
  function buildInventarioMagazzinoClientiAs400(codiceAzienda, taglieScalarini, modelliEScalarini, inventarioOrdet00fAs400) {
    var stagione, modello, articolo, colore, scalarino, inProduzione, taglie, colonneTaglie, j, l, taglia, qta, desscal,
      col = codici.colNamesToColIndexes(inventarioOrdet00fAs400[0].columnNames),
      rows = inventarioOrdet00fAs400[0].rows, r, i, ii,
      warns = [], inventario = [],
  ...
  function buildInventarioMagazzinoDisponibileAs400(codiceAzienda, taglieScalarini, modelliEScalarini, inventarioSalmodAs400) {
    var stagione, modello, articolo, colore, scalarino, taglia, qta, desscal,
      col = codici.colNamesToColIndexes(inventarioSalmodAs400[0].columnNames),
      rows = inventarioSalmodAs400[0].rows, r, i, ii,
      warns = [], inventario = [],
      mes = modelliEScalarini.lista;
  ...
  function updateGiacenzeWithInventarioAndMovimentiMagazzino(warns, giacenze, inventario, movimenti) {
    ...
    if (movimenti) {
      movimenti.rows.forEach(function (row) {
        ...
        var mm = row.doc,
          infoMm = codici.parseIdMovimentoMagazzino(mm._id),
          causale = mm.causale;
        ...
        } else if (infoMm.origine !== ids.codiceAzienda) {
          warns.push('Ignoro ' + mm._id + ' per ' + ids.codiceAzienda);
        }
      });
    }
  ...
  function updateGiacenzeWithMovimentiMagazzino(warns, giacenze, idInventario, movimenti) {
    var inventarioVuoto = {
      _id: idInventario,
      columnNames: [],
      rows: []
    };
    updateGiacenzeWithInventarioAndMovimentiMagazzino(warns, giacenze, inventarioVuoto, movimenti);
  }
      ...
      couchdb.all({
        ...
        couchdb.get(['TaglieScalarini', 'ModelliEScalarini'], function (errGet, docs) {
          ...
          updateGiacenzeAzienda = function () {
            var idInventario, azienda = rows[i].doc,
              codiceAzienda = azienda._id.split('_')[1];
            if (azienda.tipo === 'NEGOZIO') {
              idInventario = codici.idInventario(codiceAzienda, codici.TIPO_MAGAZZINO_NEGOZIO);
              couchdb.get(idInventario, function (err, inventario) {
                if (err) {
                ...
                couchdb.view('boutique_db/movimentiMagazzinoAccodati', { startkey: [codiceAzienda], endkey: [codiceAzienda, {}], include_docs: true }, function (errV, movimenti) {
                  if (errV) {
                  ...
                  if (inventario) {
                    ...
                    updateGiacenzeWithMovimentiMagazzino(allWarns, giacenze, idInventario, movimenti);
                  }
                  save();
                });
              });
