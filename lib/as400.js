...
define(['underscore', 'lib/taskutil', 'lib/couchutil', 'app/js/codici'], function (_, taskutil, couchutil, codici) {
  ...
  function buildInventarioTipo1O3As400(azienda, tipoMagazzino, taglieScalarini, modelliEScalarini, inventarioOrdet00fAs400) {
    ...
    for (; i < n; i += 1) {
      ...
      if (!desscal) {
        ...
        taglie = Object.keys(taglieScalarini[scalarino].descrizioniTaglie);
        colonneTaglie = taglieScalarini[scalarino].colonneTaglie;
  ...
  function buildInventarioTipo2As400(azienda, taglieScalarini, modelliEScalarini, inventarioSalmodAs400) {
    ...
    for (; i < n; i += 1) {
      ...
      if (!desscal) {
        ...
        colore = codici.padZero(r[col.colore], codici.LEN_COLORE);
        //TODO fare anche controllo listino As400.
        taglia = taglieScalarini[scalarino].taglie[r[col.descrizioneTaglia]];
  ...
  function comparatoreRigaInventario(a, b) {
    var stagA, stagB, aziendaA, aziendaB, tipoMagA, tipoMagB, diffStato, diffQta,
      mactA = a[0].substring(3), mactB = b[0].substring(3);
    if (mactA < mactB) {
      return -1;
    }
    if (mactA > mactB) {
    ...
    stagA = a[0].substring(0, 3);
    stagB = b[0].substring(0, 3);
    if (stagA < stagB) {
      return -1;
    }
    if (stagA > stagB) {
    ...
    diffQta = a[1] - b[1];
    return diffQta < 0 ? -1 : diffQta > 0 ? 1 : 0;
  }
      ...
      couchdb.all({
        ...
        couchdb.get(['TaglieScalarini', 'ModelliEScalarini'], function (errGet, docs) {
          ...
          var taglieScalarini = docs.rows[0].doc, modelliEScalarini = docs.rows[1].doc,
            rows = aziende.rows, i = 0, n = rows.length, azienda, allWarns = [], inventario = [], doInventario;
          ...
          function appendInventario(err, warnsAndDoc) {
            ...
            inventario.concat(warnsAndDoc[1]);
            save();
          }

          doInventario = function () {
            azienda = rows[i].doc;
            var codice = azienda._id.split('_')[1];
