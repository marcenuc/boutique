    ...
    loadFromCsvFile: function (csvFileName, azienda, couchdb, doReset, callback) {
      couchdb.get(['TaglieScalarini', 'ModelliEScalarini'], function (errDb, docs) {
        ...
        csv().fromPath(csvFileName, {
          ...
          var e = errs.join('\n'), id = 'Inventario_' + azienda, inventario;
          if (errDb) {
            return callback(e + '\n' + errDb);
          }
          if (errs.length) {
          ...
          if (doReset) {
            inventario = {
              ...
              rows: articoli.sort(comparatoreRigaInventario)
            };
            ...
            couchdb.get([id], function (errGet, oldDocs) {
              ...
              var row = oldDocs.rows[0], oldDoc = row.doc, doc = { _id: id, columnNames: columnNames };
              if (row.error && row.error !== 'not_found') {
                return callback(errGet);
              }
              if (oldDoc) {
                doc._rev = oldDoc._rev;
                doc.rows = articoli.concat(oldDoc.rows).sort(comparatoreRigaInventario);
              } else {
                doc.rows = articoli.sort(comparatoreRigaInventario);
              }
              couchutil.saveIfChanged(null, couchdb, [[], doc], callback);
            });
          }
