...
define(['underscore', 'app/js/codici', 'lib/listino', 'text!lib/etichette-template.txt', 'text!lib/etichette-outlet-template.txt'], function (_, codici, listino, templateStandard, templateOutlet) {
  'use strict';
  ...
  return {
    ...
    stampaFromGiacenze: function (couchdb, codiceAzienda, tipoMagazzino, comparatorName, templateName, callback) {
      couchdb.get(['Giacenze', 'TaglieScalarini', 'ModelliEScalarini', codici.idAzienda(codiceAzienda)], function (errDb, docs) {
        ...
        listino.getLatest(couchdb, azienda.versioneListino, function (err, listinoAzienda) {
          ...
          if (!comparators[comparatorName]) {
            return callback('Ordinamento sconosciuto');
          }
          if (!templates[templateName]) {
            return callback('Template sconosciuto');
          }
          var warns = [],
          ...
          callback(null, warns, _.template(templates[templateName], { rows: labels.sort(comparators[comparatorName]) }));
        });
      });
    },

    stampaFromMovimentoMagazzino: function (idMovimentoMagazzino, couchdb, comparatorName, templateName, callback) {
