      ...
      couchdb.get(['Scalarini', 'ModelliEScalarini'], function (errDb, docs) {
        ...
        csv().fromPath(csvFileName, {
          ...
          if (taglia[0]) {
            ...
            if (!codice) {
              return callback('Codice non valido: stagione="' + stagione + '", modello="' + modello + '", articolo="' + articolo + '", colore="' + colore + '", taglia="' + descrizioneTaglia + '"(' + taglia[1] + ')');
            }
            if (!/^\d+$/.test(giacenza)) {
          ...
          toTxt(warns, couchdb, articoli, descrizioniTaglie, listaModelli, comparators[comparatorName], callback);
