...
define(['underscore', 'fs', 'app/js/codici', 'lib/listino'], function (_, fs, codici, listino) {
  ...
  function articoliRowsToLabelRows(warns, articoli, listini, descrizioniTaglie, modelli, codiceAzienda) {
    var prezziArticolo, qtas, taglia, qta, descrizioni, label, j, colListino,
      ...
      labels = [], rows = articoli.rows, r, i, ii;
  ...
  // TODO DRY copiata da controller.js Ctrl.RicercaArticoli.
  function getFiltroSmacAz(stagione, modello, articolo, colore, aziende) {
    var toks = ['^',
      codici.dotPad(stagione, codici.LEN_STAGIONE),
      codici.dotPad(modello, codici.LEN_MODELLO),
      codici.dotPad(articolo, codici.LEN_ARTICOLO),
      codici.dotPad(colore, codici.LEN_COLORE),
      '(?:', aziende || '\\d{6}', ')',
      '$'];
  ...
  return {
    ...
    stampaFromQueryGiacenze: function (couchdb, stagione, modello, articolo, colore, aziende, comparatorName, layout, formato, callback) {
      couchdb.get(['Giacenze', 'TaglieScalarini', 'ModelliEScalarini'], function (errDb, docs) {
        if (errDb) {
          return callback(errDb);
        }
        var giacenze = docs.rows[0].doc,
          filtroSmacAz = getFiltroSmacAz(stagione, modello, articolo, colore, aziende),
          rows = giacenze.rows.filter(function (r) {
